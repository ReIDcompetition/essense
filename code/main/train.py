...
#调用dataset
image_datasets['train'] = MyDataset(root='./data/train/', datatxt='train_list.txt', mode='train', transform=data_transforms['train'])
image_datasets['val'] = MyDataset(root='./data/train/', datatxt='train_list.txt', mode='val',transform=data_transforms['val'])


# print(image_datasets['train'].images)
# print(image_datasets['val'].images)
# assert 1==False
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for batch_idx, data in enumerate(dataloaders[phase]):
                ...
                if not opt.PCB:
                    _, preds = outputs.max(1, keepdim=False)
                ...
                total += labels.size(0)
                #进度条
                utils.progress_bar(batch_idx, len(dataloaders[phase]), 'Loss: %.3f | Acc: %.3f%% (%d/%d)' % (running_loss/(batch_idx+1), 100.*float(running_corrects)/total, running_corrects, total))
...
def save_network(network, epoch_label):
    ...
    save_path = os.path.join('./model2',name,save_filename)
...
if opt.use_dense:   #4768是分类数
    model = ft_net_dense(4768)
else:
    model = ft_net(4768)
